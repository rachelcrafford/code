...
mesa <- readLAScatalog("G:/.shortcut-targets-by-id/18KzRJgC9b5miNvjWohrVBh1U8Mq9tdVP/GMUG_LiDAR/data/mesa/")
...
process_las_catalog <- function(z, i, r) { # user-defined function. can add metrics based on interest
  list(
    z80 = quantile(z, probs=0.80), # heights above 80th percentile
    z_mean = mean(z), # mean height
    coef_var = sd(z) / mean(z) * 100, # coefficient of variation
    canopy_cover = length(z[z > 2.0]) / length(z),
    understory_cover = length(z[z > 1.0 & z <= 3.0]) / length(z[z <= 3.0]) # adjust understory based on vegetation in area of interest
  )  
}
...
myfun <- function(chunk,template_path="G:/.shortcut-targets-by-id/18KzRJgC9b5miNvjWohrVBh1U8Mq9tdVP/GMUG_LiDAR/data/masks/GB_mask_3m_v1.tif") {
  ...
  # Normalize and filter point cloud
  norm <- normalize_height(las_reproj,algorithm=tin())
  filt <- filter_poi(norm, Z < 55 & Z > 0)
  
  # Compute custom pixel metrics
  custommetrics <- pixel_metrics(filt, 
                                      ~process_las_catalog(z = Z, 
                                                           i = Intensity, 
                                                           r = ReturnNumber),
                                      res=10)
...
mesa_veg <- catalog_apply(mesa, myfun)

## Exporting results

# Convert list elements to SpatRaster
rast_list <- list()
for (i in 1:length(mesa_veg)) {
  rast_list[[i]] <- rast(mesa_veg[[i]])
}
...
zmean <- (mesa_mos[[2]])
coefvar <- (mesa_mos[[3]])
canopycov <- (mesa_mos[[4]])
understorycov <- (mesa_mos[[5]])

# Save to disk
writeRaster(understorycov, filename = 'G:/.shortcut-targets-by-id/18KzRJgC9b5miNvjWohrVBh1U8Mq9tdVP/GMUG_LiDAR/deliverables/Block 5_2//understorycov.tif',
            overwrite=TRUE)
writeRaster(canopycov, filename = 'G:/.shortcut-targets-by-id/18KzRJgC9b5miNvjWohrVBh1U8Mq9tdVP/GMUG_LiDAR/deliverables/Block 5_2/canopycov.tif',
            overwrite=TRUE)
writeRaster(coefvar, filename = 'G:/.shortcut-targets-by-id/18KzRJgC9b5miNvjWohrVBh1U8Mq9tdVP/GMUG_LiDAR/deliverables/Block 5_2/coefvar.tif',
            overwrite=TRUE)
writeRaster(zmean, filename = 'G:/.shortcut-targets-by-id/18KzRJgC9b5miNvjWohrVBh1U8Mq9tdVP/GMUG_LiDAR/deliverables/Block 5_2/zmean.tif',
            overwrite=TRUE)
writeRaster(z80, filename = 'G:/.shortcut-targets-by-id/18KzRJgC9b5miNvjWohrVBh1U8Mq9tdVP/GMUG_LiDAR/deliverables/Block 5_2/z80.tif',
            overwrite=TRUE)
